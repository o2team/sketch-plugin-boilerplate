...
var webviewIdentifier = 'my-sketch';
var options = {
  identifier: 'my-sketch',
  width: 40,
  height: 600,
  frame: true,
  modal: false,
  titleBarStyle: 'hidden',
  // frame 红绿灯按钮样式  default 不展示 
  transparent: false,
...
/* harmony default export */ __webpack_exports__["default"] = (function (context) {
  ...
  toolbar.identifier = 'relay.tool.box';
  toolbar.setSpacing(10);
  toolbar.setFlipped(true);
  toolbar.setBackgroundColor(NSColor.windowBackgroundColor()); // const button =  NSButton.alloc().initWithFrame(NSMakeRect(0, 0, 40, 80))
  // button.identifier = 'upload'
  // toolbar.setViews_inGravity(button,1)
  ...
  var threadDictionary = NSThread.mainThread().threadDictionary();
  var identifier = "co.yourIdentifier";
  console.error('identifier', identifier); // If there is already a panel, prevent the plugin from running again

  if (threadDictionary[identifier]) return;
  threadDictionary.panelOpen = true;
  setupPanel(threadDictionary, identifier); // console.error('context', context)
  // console.error('document', context.document)
  // console.error('documentWindow', context.document.documentWindow())
  // console.error('documentWindow.contentView', context.document.documentWindow().contentView())
  // console.error('documentWindow.contentView.subviews', context.document.documentWindow().contentView().subviews())
  // console.error('selection', context.selection)
  // console.error('NSColor.Name', NSColor.colorNamed)
  // console.error('NSColor.windowBackgroundColor', NSColor.windowBackgroundColor)
  // sketch.UI.message("My sketch plugin2222 ! It's alive 🙌")
  // console.log('my plugin 333')
  // const webView = new BrowserWindow(options)
  // webView._webview.identifier = webviewIdentifier
  // webView.loadURL('https://docs.pfan123.com/')
  // // webView.show()
  // // console.log('webView', webView)
  // const contentView = context.document.documentWindow().contentView()
  // console.error('contentView', contentView)
  // if (!contentView) {
  //   return false;
  // }
  // // const panelBox = NSStackView.alloc().initWithFrame(NSMakeRect(0, 0, 40, 600))
  // // console.error('panelBox11111111', panelBox)
  // // panelBox.identifier = 'my-sketch-panel'
  // // panelBox.orientation = 1
  // // console.error('panelBox11111111', panelBox)
  // // panelBox.addView_inGravity(webView, 1)
  // const stageView = contentView.subviews().objectAtIndex(0)
  // const existId = stageView.subviews().find(d=>''.concat(d.identifier()) == webviewIdentifier)
  ...
  // 	const view = views[i]
  // 	if(existId){
  // 		if(view.identifier() != webviewIdentifier) finalViews.push(view)
  // 	}else{
  //     finalViews.push(view)
  //     // Change the view identifier here if you want to add your panel anywhere else 
  // 		if (!pushedWebView && view.identifier() == 'view_canvas') {
  // 			finalViews.push(webView._webview)
  // 			pushedWebView = true
  // 		}
  // 	}
  // }
  // console.error('subviews', finalViews)
...
function addButton(rect, name, callAction) {
  var button = NSButton.alloc().initWithFrame(rect); // image = getImage(rect.size, name);
...
function setupPanel(threadDictionary, identifier) {
  ...
  panel.setBackgroundColor(NSColor.whiteColor()); // panel.title = "SVGO UI"

  panel.center(); // panel.setBoundsOrigin(0, 0)
  ...
  var contentView = panel.contentView();
  var linkButton = addButton(NSMakeRect(20, 20, 161, 30), "title-1", function (e) {
    console.error(11111);
    console.error(e);
  });
  contentView.addSubview(linkButton); // COScript.currentCOScript().setShouldKeepAround_(true)
  // panel.standardWindowButton(NSWindowMiniaturizeButton).setHidden(true)
  // panel.standardWindowButton(NSWindowZoomButton).setHidden(true)
